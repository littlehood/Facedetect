...
def detect_result(image):
    ...
    try:
        ...
        return [u"检测失败，请重试"]
    # 美丑评分
    result_list.append(u"美丑打分：%d " % result["beauty"])
    result_list.append(u"美丑打分：%d " % result["beauty"])
    # 年龄
    result_list.append(u"年龄：%d " % result["age"])
    # 表情
    if result["expression_probablity"] >= 0.9:
        if result["expression"] == 0:
            result_list.append(u"表情：不笑 ")
        elif result["expression"] == 1:
            result_list.append(u"表情：微笑 ")
        elif result["expression"] == 2:
            result_list.append(u"表情：大笑 ")
    # 脸型
